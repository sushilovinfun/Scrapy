...
class MySpider(CrawlSpider):
    ...
    start_urls = ["http://genofond.org/viewforum.php?f=9"]

    rules = (
            # extract and follow the forum's page links
            Rule(SgmlLinkExtractor(restrict_xpaths="//*[@id='pagecontent']/table[2]/tr/td[2]")),
            Rule(SgmlLinkExtractor(allow='.&t=\d+'), callback='parse_topic'),
            Rule(SgmlLinkExtractor(allow='.&t=\d+&start=\d+'), callback='parse_topic', follow=True),
            Rule(SgmlLinkExtractor(deny='.&p=\S+')),
            Rule(SgmlLinkExtractor(deny='.&t=\S+&view=next')),
            Rule(SgmlLinkExtractor(deny='.&t=\S+&view=previous')),
            Rule(SgmlLinkExtractor(deny='.&mode=reply&t=\d+'))
            )
